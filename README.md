...
Local development defaults `STUDY_SHELL_STORE=memory`, `PARTICIPANT_SLOT_STORE=memory`, `CONSENT_VERSION_STORE=memory`, `SURVEY_VERSION_STORE=memory`, `OBJECTIVE_VERSION_STORE=memory`, and `RUN_LIFECYCLE_STORE=memory` so the researcher setup flow works without DynamoDB Local. Set a store variable to `dynamodb` when you want that slice to persist to its local DynamoDB table.
